...
import com.accountbook.fjy.adapter.CalendarAdapter;
...
public class CalendarDialog extends Dialog implements View.OnClickListener {
    ...
    private void addViewToLayout() {
        hsvViewList = new ArrayList<>();    //将添加进入线性布局中的TextView进行统一管理的集合
        yearList = DBManager.getYearListFromAccounttb();    //获取数据库中存储的年份
        //如果数据库中没有记录，就添加今年的记录
        if (yearList.size()==0) {
            int year = Calendar.getInstance().get(Calendar.YEAR);
            yearList.add(year);
        }
        //遍历年份并向ScrollView中添加对应个数的View
        ...
        if (selectPos == -1) {
            selectPos = hsvViewList.size()-1;   //设置当前被选中的为最近的年份
        }
        ...
        for (int i = 0; i < hsvViewList.size(); i++) {
            ...
            view.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    changeTvbg(pos);
                    selectPos = pos;
                    //获取被选中的年份，并让下面的GridView显示数据源发生变化
